...
import com.example.pokemon.databinding.FragmentLeftBinding
...
class LeftFragment : Fragment() {
    ...
    database.child("entrenadores").child("001").get().addOnSuccessListener { record->
        val json= JSONObject(record.value.toString())
        Log.d("sds","${record.value}")
        binding.nombre.setText(json.getString("nombre").toString())
        binding.nickname.setText(json.getString("apodo").toString())
        binding.nivel.setText(json.getString("nivel").toString())
        binding.pokemons.setText(json.getString("atrapados").toString())
        binding.pokemons.setText(json.getString("origen").toString())
    }
    return binding.root
}
}